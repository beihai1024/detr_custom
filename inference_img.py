...
def plot_result(pil_img, prob, boxes, save_name=None, imshow=False, imwrite=False):
    ...
    if len(prob) == 0:
        ...
        if imwrite:
            if not os.path.exists("/content/result/pred_no"):
                os.makedirs("/content/result/pred_no")
            cv2.imwrite(os.path.join("/content/result/pred_no", save_name), opencvImage)
        return
    ...
    if imwrite:
        if not os.path.exists("/content/result/pred"):
            os.makedirs('/content/result/pred')
        cv2.imwrite('/content/result/pred/{}'.format(save_name), opencvImage)
...
if __name__ == "__main__":
    ...
    files = os.listdir("/content/dataset/test")

    for file in files:
        img_path = os.path.join("/content/dataset/test", file)
